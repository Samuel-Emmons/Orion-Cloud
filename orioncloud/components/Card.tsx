...
export const Card = ({file, className}: {file: CardFile; className?: string}) => {
    const ownerName = typeof file.owner === "object" ? file.owner?.fullName : undefined;

    return <article className={cn("group relative flex h-full min-w-0 flex-col gap-3 rounded-2xl border border-gray-200 bg-white p-3 text-gray-900 shadow-sm transition hover:border-brand hover:shadow-md", className)}>
